...
Node *order_n_teams(int n)
{
  ...
  for (int i=1; i<n; i++) {
	...
	//So this is the insert in the middle huh. Gotcha

	for(int a = 0; a<T->size; a++){
		//L-L-L-L-W-W-W-W
		//      Here
		if(did_x_beat_y(select(T,a)->key, i) && did_x_beat_y(i, select(T,a+1)->key)){
			T = insert_keep_balanced(T, i, a+1);	
			break;
		}
...
int main(void)
{
  ...
  cout << "Inorder traversal:  "; print_inorder(T); cout << "\n\n";
  cout << "Insert Done, Doing remove" << endl;
  // test remove
  ...
  if (S == NULL || S->size != N)
    ...
    if (N<=1000) { cout << "Team ordering: "; print_inorder(S); cout << "\n"; } 
    for (int i=0; i<N-1; i++) {
      Node *x = select(S, i);
      Node *y = select(S, i+1);
